...
interface OData {
  planName: string; planDescription: string
...
export function OnboardingWrapper({ userId, onComplete }: Props) {
  ...
  const [data, setData] = useState<OData>({
    planName: 'Mein Trainingsplan', planDescription: '',
    trainingDays: DEFAULT_DAYS, exercises: DEFAULT_EXERCISES,
    caloriesTraining: 2800, proteinTraining: 180, carbsTraining: 320, fatTraining: 80,
    caloriesRest: 2200, proteinRest: 160, carbsRest: 200, fatRest: 70,
  ...
  const handleSubmit = async () => {
    ...
    try {
      await supabase.from('profiles').update({
        main_goal: data.mainGoal, rest_days: data.restDays, onboarding_done: true,
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0a', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, padding: '24px 20px', overflowY: 'auto' }}>
        {step === 1 && <Step2PlanName planName={data.planName} planDescription={data.planDescription} onChange={(f, v) => set(f, v)} />}
        {step === 2 && <Step3TrainingDays days={data.trainingDays} onChange={days => set('trainingDays', days)} />}
        {step === 3 && (
          <Step4Exercises
        ...
        {step === 6 && (
          <Step7GoalSummary
            ...
            loading={saving} onGoalChange={goal => set('mainGoal', goal)} onSubmit={handleSubmit}
          />
        )}
      </div>
